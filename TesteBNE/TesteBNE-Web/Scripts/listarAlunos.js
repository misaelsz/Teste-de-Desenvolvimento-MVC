...
function listar(ObjPessoa) {
	...
	$.each(ObjPessoa, function (chave, valor) {
		...
		$("#listaAlunos tbody").append(
			...
			"<td><button type='button' onclick='AbreVinculadas(" + JSON.stringify(valor.ID) + ");' class='btn btn-success'>Listar</button></td>"
		);
	});
}
...
function AbreVinculadas(id) {
	var url = location.href;
	url = url.split("/")[0] + "/Disciplina/DisciplinasVinculadas/" + id;
